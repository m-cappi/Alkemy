...
CREATE TABLE IF NOT EXISTS `Alkemy`.`transactions` (
  ...
  `creation_date` DATETIME GENERATED ALWAYS AS (current_timestamp()) VIRTUAL,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Alkemy`.`view_transactions` (`'Date'` INT, `Concept` INT, `Category` INT, `'Type'` INT);
...
SELECT a.creation_date as 'Date', a.concept as Concept, b.categ_name as Category, c.name_type as 'Type' FROM transactions a
